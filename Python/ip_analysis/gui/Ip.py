...
from gui.IpTools import make_net_mask, calculate_all_parameters

class Ip(QWidget):
    def __init__(self):
        super().__init__()
        self.label = QLabel("Network ve BroadCast ID Hesaplama ")

        self.label.setStyleSheet("font: 18pt Times New Roman")

        self.ip_input = QLineEdit()
        self.ip_input.setPlaceholderText("Ip Adresini giriniz...")
        ...
        self.max_hosts_label = QLabel("Max. Host")
    ...
    def check_ip(self, text):
        ...
        if text == "":
            QMessageBox.about(self, "Uyarı!", "Ip adresi alanı boş bırakılamaz!")
            return False
        elif len(list_of_parts) != 4:
            QMessageBox.about(self, "Uyarı!", "Ip adresi dört bölüm içeriyor!")
            return False
        else:
            for part in list_of_parts:
                if not part.isdigit():
                    QMessageBox.about(self, "Uyarı!", "IP adresi yalnızca rakam içermeli!")
                    return False
                if int(part) > 255:
                    QMessageBox.about(self, "Uyarı!", "Bir ip adresinin her kısmı 256'dan küçük olmalıdır!")
                    return False
        return True
